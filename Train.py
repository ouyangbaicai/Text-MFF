...
class NetTrain:
    def __init__(self,
                 data_path='D:\\ouyangbaicai\\MSI-DTrans\\Datasets\\Train&Valid\\MFFdatasets',
                 set_size=5000,
                 batchsize=8,
                 epochs=200,
                 lr=0.0002,
        ...
        for epoch in range(epochs):
            ...
            for A, B, GT, DM in train_loader_tqdm:
                ...
                with torch.autocast(device_type=self.DEVICE, dtype=torch.float32):
                    Pre_A = model(A)
                    Pre_B = model(B)
                    # Fused_img = model(Pre_A, Pre_B, forward=False)
                    # fea = Pre_A + Pre_B
                    fea = torch.max(Pre_A, Pre_B)
                    Fused_img = model(fea, None, forward=False)
                    loss = self.MixLoss(Fused_img, GT)
            ...
            with torch.no_grad():
                ...
                for A, B, GT, DM in valid_loader_tqdm:
                    Pre_A = model(A, forward=True)
                    Pre_B = model(B, forward=True)
                    # Fused_img = model(Pre_A, Pre_B, forward=False)
                    # fea = Pre_A + Pre_B
                    fea = torch.max(Pre_A, Pre_B)
                    Fused_img = model(fea, None, forward=False)
